...
namespace Course.Services.Catalog.Services.Concrede
{
    public class CategoryService:ICategoryService
    {
        ...
        public async Task<Response<CategoryDto>> CreateAsync(CategoryDto categoryDto)
        {
            ...
            return Response<CategoryDto>.Success(categoryResult, 200);
        }

        public async Task<Response<List<CategoryDto>>> GetAllAsync()
        {
            var categories = await _categoryCollection.Find(category => true).ToListAsync();
            var categoriesResult = _mapper.Map<List<CategoryDto>>(categories);

            return Response<List<CategoryDto>>.Success(categoriesResult, 200);
        }
